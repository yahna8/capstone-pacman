...
public class GhostStateManager : MonoBehaviour
{
    ...
    [Header("Movement")]
    [SerializeField] private bool useMazePathing = true;
    ...
    [Header("Visual State")]
    [SerializeField] private Color frightenedColor = new Color(0.1f, 0.25f, 1f, 1f);
    [SerializeField] private Color eatenColor = Color.white;
    ...
    private void Awake()
    {
        ...
        if (ghostRenderer != null && ghostRenderer.material != null)
            normalColor = GetRendererColor(ghostRenderer.material, normalColor);
    ...
    private void OnValidate()
    {
        ...
        pinkyLookAheadDistance = Mathf.Max(0f, pinkyLookAheadDistance);
        inkyLookAheadDistance = Mathf.Max(0f, inkyLookAheadDistance);
        clydeScatterDistance = Mathf.Max(0f, clydeScatterDistance);
        playerHeadingVelocityThreshold = Mathf.Max(0f, playerHeadingVelocityThreshold);
        FrightenedSeconds = Mathf.Max(0.1f, FrightenedSeconds);
    }
    ...
    public void ApplyNormalVisual()
    {
        ApplyRendererColor(normalColor);
    }

    public void ApplyFrightenedVisual()
    {
        ApplyRendererColor(frightenedColor);
    }
    ...
    {
        SetMoveTarget(GetMovementTarget(worldTarget));
        MoveTowardCurrentTarget(speed);
        return HasArrivedAt(worldTarget);
    }

    private Vector3 GetMovementTarget(Vector3 worldTarget)
    {
        if (!useMazePathing || MazeBuilder.Instance == null)
            return worldTarget;

        if (MazeBuilder.Instance.TryGetNextPathStep(transform.position, worldTarget, out Vector3 pathStep))
            return pathStep;

        return worldTarget;
    }

    private bool HasArrivedAt(Vector3 worldTarget)
    {
        Vector3 arrivalTarget = worldTarget;
        if (useMazePathing && MazeBuilder.Instance != null &&
            MazeBuilder.Instance.TryGetNearestWalkablePosition(worldTarget, out Vector3 walkableTarget))
        {
            arrivalTarget = walkableTarget;
        }

        Vector3 toTarget = arrivalTarget - transform.position;
        toTarget.y = 0f;
        return toTarget.sqrMagnitude <= arriveDistance * arriveDistance;
    }
    ...
    private bool MoveTowardCurrentTarget(float speed)
    {
        ...
        if (controller != null)
        {
            ...
            Vector3 horizontalMove = dir * Mathf.Min(maxDistance, toTarget.magnitude);
        ...
        {
            float maxDistance = speed * Time.deltaTime;
            transform.position += dir * Mathf.Min(maxDistance, toTarget.magnitude);
        }
    ...
    private static Color GetRendererColor(Material material, Color fallback)
    {
        if (material.HasProperty("_BaseColor"))
